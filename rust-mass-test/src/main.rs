...
async fn run_producers_with_ui(config: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let metrics = Arc::new(GlobalMetrics::new(config.num_producers));
    ...
    for producer_id in 0..config.num_producers {
        ...
        let producer_metrics = metrics.producers[producer_id].clone();

        let handle: JoinHandle<Result<(), Box<dyn std::error::Error + Send + Sync>>> =
            tokio::spawn(async move {
            producer::run_producer(producer_id, config_clone, Arc::new(producer_metrics))
                .await
        });
    ...
    loop {
        // Draw metrics
        terminal.draw(|f| draw_metrics_screen(f, &metrics, start_time.elapsed()))?;
    ...
    eprintln!("Total messages published: {}", metrics.get_total_published());
...
async fn run_producers(config: &Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Wait for all producers (they run indefinitely)
    for handle in handles {
        let _ = handle.await;
    }
